...
function buildTestCatalog() {
  ...
  catalog.set('black-beautiful-and-baby', {
...
test('rejects quantity 0', () => {
  const catalog = buildTestCatalog();
  const result = validateCheckoutItems(
    { items: [{ slug: 'florida-beach-and-baby', quantity: 0 }] },
    catalog,
  );
  assert.equal(result.ok, false);
});
...
test('rejects a fractional quantity', () => {
  const catalog = buildTestCatalog();
  const result = validateCheckoutItems(
    { items: [{ slug: 'florida-beach-and-baby', quantity: 1.5 }] },
    catalog,
  );
  assert.equal(result.ok, false);
});
...
test(`rejects a quantity above ${MAX_QUANTITY_PER_ITEM}`, () => {
  const catalog = buildTestCatalog();
  const result = validateCheckoutItems(
    { items: [{ slug: 'florida-beach-and-baby', quantity: MAX_QUANTITY_PER_ITEM + 1 }] },
    catalog,
  );
  assert.equal(result.ok, false);
});
...
test('accepts a quantity of exactly the per-item maximum', () => {
  const catalog = buildTestCatalog();
  const result = validateCheckoutItems(
    { items: [{ slug: 'florida-beach-and-baby', quantity: MAX_QUANTITY_PER_ITEM }] },
    catalog,
  );
  assert.equal(result.ok, true);
});
  ...
  const result = validateCheckoutItems(
    {
      items: [
        ...
        { slug: 'florida-beach-and-baby', quantity: 3 },
      ],
    },
    catalog,
  );
...
test('rejects duplicate slug entries whose merged quantity exceeds the per-item cap', () => {
  ...
  const result = validateCheckoutItems(
    {
      items: [
        { slug: 'florida-beach-and-baby', quantity: 15 },
        { slug: 'florida-beach-and-baby', quantity: 15 },
      ],
    },
    catalog,
...
test(`rejects a cart whose total quantity exceeds ${MAX_TOTAL_CART_QUANTITY}`, () => {
  ...
  const result = validateCheckoutItems(
    {
      items: [
        { slug: 'florida-beach-and-baby', quantity: 20 },
        { slug: 'black-beautiful-and-baby', quantity: 20 },
        { slug: 'florida-beach-and-baby', quantity: 5 },
      ],
    },
    catalog,
  );
  // florida-beach-and-baby merges to 25, which also exceeds the per-item cap,
  // so this should fail — but even if it didn't, the cart total (45) exceeds
...
test('uses the server-resolved Price ID, never anything from the client', () => {
  ...
  assert.equal(result.items[0].title, 'Florida, Beach & Baby');
});
